...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, padding=1):
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer5 = self._make_layer(
            block, 64, 1, shortcut_type, stride=2)
        self.layer6 = self._make_layer(
            block, 64, 1, shortcut_type, stride=2)
        self.layer7 = self._make_layer(
            block, 64, 1, shortcut_type, down_kernel_size=(1, 3, 3), stride=2, padding=0)
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, down_kernel_size=1, stride=1, padding=1):
        downsample = None
...
@registry.BACKBONES.register('resnet3d')
def resnet3d(cfg, pretrained=True):
    model = resnet50(sample_duration=32,
                     spatial_size=cfg.INPUT.IMAGE_SIZE)
