...
class ResNet3dSlowFast(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_channels = []
        if cfg:
            self.out_channels = cfg.MODEL.PRIORS.FEATURE_MAPS
        ...
        self.avg_pool = nn.AvgPool3d((4, 1, 1))
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            msg = f'Loading model from: {self.pretrained}'
    ...
    def load_checkpoint(self):
        ...
        state_dict_slow = OrderedDict()
        state_dict_fast = OrderedDict()
        for k, v in state_dict.items():
            if k.startswith('backbone'):
                k = k.replace('backbone.', '')

                if k.startswith('slow_path'):
                    state_dict_slow[k.replace('slow_path.', '')] = v
                elif k.startswith('fast_path'):
                    state_dict_fast[k.replace('fast_path.', '')] = v
        self.slow_path.load_state_dict(state_dict_slow, strict=False)
        self.fast_path.load_state_dict(state_dict_fast, strict=False)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.slow_path.res_layers):
            ...
            if self.slow_path.lateral:
                ...
                x_slow = torch.cat((x_slow, x_fast_lateral), dim=1)
                if len(self.out_channels):
                    if x_slow.shape[3] in self.out_channels:
                        x_slow_avg = self.avg_pool(x_slow)
                        features.append(x_slow_avg)
...
class Backbone(nn.Module):
    def __init__(self, pretrained, cfg):
        ...
        self.backbone = ResNet3dSlowFast(pretrained, cfg)
        self.conv1 = nn.Conv3d(in_channels=2560,
                               out_channels=256,
                               kernel_size=(1, 3, 3),
                               stride=(1, 2, 2),
                               padding=(0, 1, 1))

        self.conv2 = nn.Conv3d(in_channels=256,
                               out_channels=256,
                               kernel_size=(1, 3, 3),
                               stride=(1, 2, 2),
                               padding=(0, 1, 1))
    ...
    def forward(self, x):
        ...
        x_slow = self.conv1(x_slow)
        features.append(x_slow)
        x_slow = self.conv2(x_slow)
        features.append(x_slow)
        x_slow = self.conv3(x_slow)
        features.append(x_slow)
        return features


@registry.BACKBONES.register('slowfast')
def slowfast(cfg, pretrained="../checkpoints/slowfast_r50_256p_4x16x1_256e_kinetics400_rgb_20200728-145f1097.pth"):
    return Backbone(pretrained, cfg)


if __name__ == '__main__':
    from ssd.config import cfg
    cfg.merge_from_file("/home/scj/Projects/Code/0.Frameworks/3.Video-Understanding/2.VID/22-09-25-SSD-3D/configs/r2plus1d_ssd300_coco_vid.yaml")
    cfg.freeze()
    # model = ResNet3dSlowFast(pretrained="checkpoints/slowfast_r50_256p_4x16x1_256e_kinetics400_rgb_20200728-145f1097.pth",
    #                          cfg=cfg)
    model = Backbone(pretrained="/home/scj/Projects/Code/0.Frameworks/3.Video-Understanding/2.VID/22-10-24-SSD-3D-New-VID/checkpoints/slowfast_r50_256p_4x16x1_256e_kinetics400_rgb_20200728-145f1097.pth",
                     cfg=cfg)
    # print(model)
