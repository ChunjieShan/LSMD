...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", block_size=[16, 16]):
        super().__init__()
        self.self_attn = ISABlock(d_model, nhead, dropout=dropout, block_size=block_size)
    ...
    def forward(self, src, pos):
        ...
        src = src.contiguous().view(n, c, t * h * w).permute(0, 2, 1)
        src2 = src2.contiguous().view(n, c, t * h * w).permute(0, 2, 1)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", block_size=[16, 16]):
        super().__init__()
        self.self_attn = ISABlock(d_model, nhead, dropout=dropout, block_size=block_size)
        self.multihead_attn = ISABlockCA(d_model, nhead, dropout=dropout, block_size=block_size)
        self.block_size = block_size
