...
class BoxPredictor(nn.Module):
    ...
    def forward(self, features):
        ...
        for feature, cls_header, reg_header in zip(features, self.cls_headers, self.reg_headers):
            ...
            # single_head_cls_feat = cls_header(feature).contiguous()
            # single_head_box_feat = reg_header(feature).contiguous()

            # print(True in torch.isnan(single_head_cls_feat))
            # print(True in torch.isnan(single_head_box_feat))
            # cls_logits.append(single_head_cls_feat)
            # bbox_pred.append(single_head_box_feat)

            # cls_logits.append(cls_header(feature).transpose(1, 2).transpose(2, 3).contiguous())
            # bbox_pred.append(reg_header(feature).transpose(1, 2).transpose(2, 3).contiguous())
        ...
        # print(True in torch.isnan(cls_logits))
        # print(True in torch.isnan(bbox_pred))
...
class SSDBoxPredictor(BoxPredictor):
    def cls_block(self, level, out_channels, boxes_per_location):
        return nn.Conv2d(out_channels, boxes_per_location * self.cfg.MODEL.NUM_CLASSES, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        # return nn.Conv3d(out_channels,
    ...
    def reg_block(self, level, out_channels, boxes_per_location):
        return nn.Conv2d(out_channels, boxes_per_location * 4, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
        # return nn.Conv3d(out_channels,
