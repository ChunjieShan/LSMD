...
class DarkVIDDataset(Dataset):
    ...
    def __init__(self,
        ...
        #                 'common_cross', 'in_ex_cross', 'vertical', 'plaque']
        # self.classes = ['__background__', 'cross']
    ...
    def __getitem__(self, index):
        ...
        curr_video_clip = data_clip
        curr_gt = self.training_label_list[index]
        start_idx = int(curr_video_clip[0].split(
            os.path.sep)[-1].split(".")[0].split("_")[-1])
        ...
        frame_t2_idx = last_frame_idx - 2
        if frame_t2_idx == 0:
            random_sample_idxes = np.array([0])
        else:
            random_sample_idxes = np.arange(0, frame_t2_idx)

        if len(random_sample_idxes) < self.sequence_num * self.frame_len:
            padding = self.sequence_num * self.frame_len - len(random_sample_idxes)
            random_sample_idxes = np.pad(random_sample_idxes, (0, padding), 'edge')
        else:
            np.random.shuffle(random_sample_idxes)
            sampling_idxes = random_sample_idxes[:self.frame_len * self.sequence_num]
            random_sample_idxes = random_sample_idxes[sampling_idxes]
            random_sample_idxes.sort()

        # print(random_sample_idxes)
        prev_img_names = [curr_video_name + "/img_" + "{:05d}.jpg".format(idx) for idx in random_sample_idxes]
        prev_img_names_groups = [prev_img_names[i: i + self.frame_len] for i in range(0, len(prev_img_names), self.frame_len)]

        prev_np_imgs_list = []
        for prev_img_names_group in prev_img_names_groups:
...
if __name__ == '__main__':
    ...
    dataset = DarkVIDDataset(
        data_dir="/mnt/h/20230718/images/train",
        label_dir="/mnt/h/20230718/labels",
        frame_len=3,
        transform=transform,
