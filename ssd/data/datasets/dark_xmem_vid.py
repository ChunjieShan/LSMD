...
from ssd.data.transforms import build_transforms, build_prev_clip_transform
...
class DarkVIDDataset(Dataset):
    ...
    def __init__(self, data_dir, label_dir, frame_len=None, transform=None, prev_transform=None, target_transform=None,
                 remove_empty=False, is_random=False, is_overlap=False):
    ...
    def __getitem__(self, index):
        ...
        if start_idx == 0:
            prev_video_clip = curr_video_clip
        else:
            if self.is_random:
                if self.remove_empty:
                    random_select_idx = random.randint(1, start_idx)
                    prev_video_clip = self.training_file_list[index - random_select_idx]
                    # check if it gets a video clip from a different video
                    if prev_video_clip[0].split(os.path.sep)[0] != curr_video_clip[0].split(os.path.sep)[0]:
                        prev_video_clip = curr_video_clip
                else:
                    prev_video_clip = self.training_file_list[index - 1]
                    if prev_video_clip[0].split(os.path.sep)[0] != curr_video_clip[0].split(os.path.sep)[0]:
                        prev_video_clip = curr_video_clip
            else:
                prev_video_clip = self.training_file_list[index - 1]
        ...
        if self.transform:
            ...
            for idx, img in enumerate(prev_img_list):
    ...
    def __len__(self):
        return len(self.training_file_list)
    ...
    def allocate_consecutive_samples(self):
        ...
        for i in range(len(gt_list) - 1):
            prev_images = img_names_list[i]
            prev_video_name = prev_images[0].split(os.path.sep)[0]
            curr_images = img_names_list[i + 1]
            ...
            if prev_video_name == curr_video_name:
                ...
                # TODO
                img_names_pairs_list.append(curr_images)
...
if __name__ == '__main__':
    ...
    dataset = DarkVIDDataset(
        data_dir="/home/scj/Code/Data/2.Carotid-Artery/2.Labelled-Data/2.OD-Labels/More-Frames-Data/20230201/20230201_cross/images/train",
    ...
    for i, batch in enumerate(dataset):
        ...
        prev = data["prev"]
        curr = data["curr"]
        assert prev.shape[0] == 3, f"[E] Previous clip error, expected 3, got {prev.shape[0]}"
