...
# Global Constants
# 数据处理输入输出参数
ROOTDIR = 'E:\\Data\\PSAX-A\\20220810\\'
ROOTDIR_LABEL = ROOTDIR + 'PSAX_A_video_label/'
ROOTDIR_VIDEO = ROOTDIR + 'original_data/'
ROOTDIR_DATASET = ROOTDIR + 'Dataset_of_ssd_v4/'
...
class Label2CoCo(object):
    def __init__(self, paths, dataset_dir, sample_dir, anno_dir, video_paths=None, newimgsize=None, overwrite=True):
        ...
        self.class_id = 1
        self.class_ids = {'PSAXA_HEART': 1}
        # self.class_ids = {'A':1,'AV':2,'LA':3,'PA':4,'RA':5,'RVOT':6}
        # self.class_mapping = {'DSJ': 'A', 'PSAXGV_AV': 'AV', 'PSAXGV_LA': 'LA',
        #                       'PSAXGV_PA': 'PA','PSAXGV_RA': 'RA','PSAXGV_RVOT': 'RVOT',
        #                       'PSAXGV_PA ': 'PA'}
        # self.color_mapping = [(0, 0, 0), (0, 255, 255), (255, 0, 255), (255, 255, 0), (128, 192, 255), (192, 255, 128),
        #                       (255, 128, 192)]
        self.class_mapping = {'PSAXA_HEART': 'PSAXA_HEART'}
        self.color_mapping = [(0, 0, 0), (0, 255, 0)]
        ...
        self.target = sample_dir.split('/')[0].split('\\')[0]
    ...
    def lable_and_img_to_coco(self):
        img_idx = 0
        ann_idx = 0
        ann_class_names = []
        for i in range(len(self.video_paths)):
            ...
            for label_i in range(len(self.label_paths[i])):
                label = pd.read_csv(self.label_paths[i][label_i])
...
def split_dataset(datadir,labeldir,datasetdir,label_names,dataend='.avi',labelend='.csv',
    ...
    print(label_dirs)
    # 求各类标注文件与视频文件名的交集
    label_lists = [dira_with_dirb(label_dir, datadir, fileflag='same', ignore_strs=['.csv','.avi'], suffix='')
...
if __name__ == '__main__':
    ...
    label_names = ['PSAX_A']
    # label_names = ['A','AV','LA','PA','RA','RVOT']
    dataset = split_dataset(datadir=ROOTDIR_VIDEO, labeldir=ROOTDIR_LABEL, datasetdir=ROOTDIR_DATASET,
                            label_names=label_names,dataend='.avi',labelend='.csv',trainval_percent=0.8,train_percent=0.8,seed=0)
